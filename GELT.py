...
class GELT(nn.Module):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.encoder = ENR_Encoder()
        self.etl = ETL()

    def forward(self, inputs):
        enc_out = self.encoder(inputs)
        enr_embed = self.etl(enc_out)

        return enr_embed


class ENR_Encoder(nn.Module):
    '''
    inputs --- [N, T_p, T_v]  T_p: length of phoneme sequence, T_v: size of vectors
    ...
    def __init__(self):
        ...
        out_channels = self.calculate_channels(hp.num_enr_vecs, 3, 2, 1, K)
    ...
    def forward(self, inputs):
        N = inputs.size(0)
        out = inputs.view(N, 1, -1, hp.num_enr_vecs)  # [N, 1, T_p, n_vecs]
        print("ref encoder out1:", out.size())

        for conv, bn in zip(self.convs, self.bns):
            out = conv(out)
            out = bn(out)
            out = F.relu(out)  # [N, 128, T_p//2^K, n_mels//2^K]
        
 #       print("ref encoder out2:", out.size())

        out = out.transpose(1, 2)  # [N, T_p//2^K, 128, n_vecs//2^K]
        
  #      print("ref encoder out3:", out.size())

        T = out.size(1)
        N = out.size(0)
        out = out.contiguous().view(N, T, -1)  # [N, T_p//2^K, 128*n_vecs//2^K]
   #     print("ref encoder out4:", out.size())


        memory, out = self.gru(out)  # out --- [1, N, E//2]
        print("ref encoder out:", out.size())

     #   print("ref encoder out 6:", out.squeeze(0).size())
...
class ETL(nn.Module):
    ...
    def forward(self, inputs):
        ...
        print("ETL  query", query.size())

        keys = torch.tanh(self.embed).unsqueeze(0).expand(N, -1, -1)  # [N, token_num, E // num_heads]
        
        print("ETL keys", keys.size())
        
        enr_embed = self.attention(query, keys)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key):
        ...
        print("multihead query:", querys.size())
        #print("multihead query length",querys[0][0])
       # print("multihead keys:", keys.size())
