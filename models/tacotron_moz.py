...
class Attention(nn.Module) :
    ...
    def forward(self, encoder_seq, query, t) :

        print("encoder seq", encoder_seq.size())
        # Transform the query vector
        query_proj = self.W(query).unsqueeze(1) #Wqt
        print("query", query_proj.size())

        # Compute the scores

        u = self.v(torch.tanh(encoder_seq + query_proj)) #vtanh(Wqt + Vxn) - Paper has bias = true
        print("alignment size", u.size())
...
class LSA(nn.Module):
    def __init__(self, attn_dim, kernel_size=31, filters=32):
        ...
        self.ta = nn.Linear(attn_dim *2, 1, bias=True)
    ...
    def init_attention(self, encoder_seq_proj) :
        ...
        self.alpha = torch.cat([torch.ones([b, 1]),torch.zeros([b, t])[:, :-1] + 1e-7], dim=1).cuda()  #new
        self.u = (0.5 * torch.ones([b, 1])).cuda() #new

    def forward(self, encoder_seq_proj, query, t , generating):

        if t == 0 : self.init_attention(encoder_seq_proj)

        processed_query = self.W(query).unsqueeze(1)
        print("processed query should be [N,1,attn_dim]", processed_query.size())

        location = torch.cat([self.cumulative.unsqueeze(1), self.attention.unsqueeze(1)], dim=1)
        print("location should be [N,tmax,filters]", location.size())

        processed_loc = self.L(self.conv(location).transpose(1, 2))
        print("processed location should be [N,tmax,attn_dims]", processed_loc.size())

        alignment = self.v(torch.tanh(processed_query + encoder_seq_proj + processed_loc))
        alignment = alignment.squeeze(-1)

        # Smooth Attention
        scores = torch.sigmoid(alignment) / torch.sigmoid(alignment).sum(dim=1, keepdim=True)
        # scores = F.softmax(u, dim=1)
        print("alignments", scores.size())
        self.cumulative += scores

        # Forward attention
        prev_alpha = F.pad(self.alpha[:, :-1].clone(),(1, 0, 0, 0)).cuda() #yamagishi has 0,0,1,0
        # compute transition potentials
        alpha = (((1 - self.u) * self.alpha.clone().cuda() + self.u * prev_alpha) + 1e-8) * scores
        # force incremental alignment - eval
        if generating:
        ...
        context = torch.bmm(self.alpha.unsqueeze(1), encoder_seq_proj)
        print("context vec should be [N,1,memory_dim]", context.size())
        context = context.squeeze(1)
        print("context vec should be [N,memory_dim]", context.size())

        # compute transition agent
        ta_input = torch.cat([context, query.squeeze(1)], dim=-1)
        self.u = torch.sigmoid(self.ta(ta_input))

        self.attention = self.alpha

        return context, self.attention
...
class GST(nn.Module):
    ...
    def __init__(self,ref_enc_filters ):
        ...
        self.ref_encoder = ReferenceEncoder(ref_enc_filters)
    ...
    def forward(self, inputs):
        ...
        print("USING REF MEL")
        style_embed = self.ref_encoder(inputs)
...
class ReferenceEncoder(nn.Module):
    ...
    def forward(self, inputs):
        ...
        final_state =  mem.squeeze(0)  # [N, 128]
        final_embed = self.projection(final_state) #[N, 256]

        print("final ref embedding size", final_embed.size())
        final_embed = torch.tanh(final_embed) # Tanh activation

        return final_embed  #original GST code uses memory and not output
...
class Decoder(nn.Module) :
    def __init__(self, n_mels, decoder_dims, lstm_dims) :
        ...
        self.attn_rnn = nn.GRUCell(decoder_dims + decoder_dims // 4, decoder_dims)
        self.rnn_input = nn.Linear(decoder_dims * 2, lstm_dims)
    ...
    def forward(self, encoder_seq, encoder_seq_proj, prenet_in,
        ...
        print("attn_hidden", attn_hidden.size())

        print("pre_net_in", prenet_in.size())

        # PreNet for the Attention RNN
        prenet_out = self.prenet(prenet_in)
        print("prenet out", prenet_out.size())
        print("context_vec", context_vec.size())

        # Compute the Attention RNN hidden state
        attn_rnn_in = torch.cat([context_vec, prenet_out], dim=-1)
        print("attn_rnn", attn_rnn_in.size())

        attn_hidden = self.attn_rnn(attn_rnn_in.squeeze(1), attn_hidden)
        print("attn_hidden", attn_hidden.size())

        # Compute the attention scores
        context_vec, attention_weights = self.attn_net(encoder_seq_proj, attn_hidden, t, self.generating)

        # this moved to Attention Layer
        # Dot product to create the context vector
        #context_vec = torch.bmm(scores.unsqueeze(1), encoder_seq )
        #context_vec = context_vec.squeeze(1)
        #print("context vec", context_vec.size())

        # Concat Attention RNN output w. Context Vector & project
        x = torch.cat([context_vec, attn_hidden], dim=1)


        print("rnn project", x.size())
        x = self.rnn_input(x)
        ...
        return mels, attention_weights, hidden_states, cell_states, context_vec


class Tacotron(nn.Module) :
    def __init__(self, embed_dims, num_chars, encoder_dims, decoder_dims, n_mels, fft_bins, postnet_dims,
                 encoder_K, lstm_dims, postnet_K, num_highways, dropout, ref_enc_filters):
        super().__init__()
        self.n_mels = n_mels
        ...
        self.ref_enc_filters = ref_enc_filters
        self.gst = GST(self.ref_enc_filters)
    ...
    def forward(self, x, m, generate_gta=False) :
        ...
        style_embed  = style_embed.unsqueeze(1) # [N, 1, 256]
        expanded = style_embed.expand_as(encoder_seq).cuda()

        print("expanded", style_embed.size())

	# concatenate embeddings

        final_enc_seq = torch.cat([encoder_seq,expanded],dim = 2)



        print("seq concatenate",final_enc_seq.size()) # [N, 512]


        encoder_seq_proj = final_enc_seq
        encoder_seq = encoder_seq_proj
    ...
    def generate(self, x,ref_m, steps=2000) :
        ...
        batch_size, n_mels , mel_steps  = ref_m.size()
        ...
        encoder_seq = self.encoder(x)
        # Change shape
        m_gst = ref_m.view(batch_size, mel_steps, n_mels)
